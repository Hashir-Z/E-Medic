...
namespace Semester_Project
{
    public partial class Tests : Form
    {
        ...
        private void pDiabetes_MouseClick(object sender, MouseEventArgs e)
        {
            if (cBDiabetes.Checked)
            {
        ...
        private string getTestList()
        {
            ...
            if (cBDiabetes.Checked)
            {
                ...
                Run = true;
                TestNames += "tName='Diabetes'";

                // Get Test Price
                sql = "SELECT tPrice FROM Test WHERE tName='Diabetes'";
                SqlCommand command = new SqlCommand(sql, cnn);
                SqlDataReader dataReader = command.ExecuteReader();

                while (dataReader.Read())
                {
                    TestTotal += "Diabetes: " + String.Format("{0:n0}", dataReader.GetInt32(0)) + "\n";
                }
                dataReader.Close();
                dataReader.Dispose();
            }
