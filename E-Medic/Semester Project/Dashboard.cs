    ...
    public partial class Dashboard : Form
    {
        ...
        private void FillCalendar()
        {
            ...
            while (dataReader.Read())
            {
                ...
                if (DateTime.Parse(DateTime.Now.Date.ToString("dd/MM/yy")) > DateTime.Parse(adate))
                {
                    appointment.Enabled = false;
                    appointment.EventColor = System.Drawing.Color.FromArgb(128, 128, 128);
                }
                calTable.AddEvent(appointment);
            }

            cnn = new SqlConnection(connetionString);
            cnn.Open();
            sql = "SELECT aDate FROM dAppointment WHERE Payment='Pending' and pID='" + pID + "'";
            command = new SqlCommand(sql, cnn);
            ...
            while (dataReader.Read())
            {
                if (DateTime.Parse(DateTime.Now.Date.ToString("MM/dd/yy")) < DateTime.Parse(dataReader.GetString(0)))
                {
                    AppRecCount++;
                }
            }
            lPendApp.Text = Convert.ToString(AppRecCount);
            ...
            lReports.Text = Convert.ToString(ReportCounts);
            ...
            while (dataReader.Read())
            {
                AmountPaid = AmountPaid + dataReader.GetInt32(1);
            }
            lAmntPaid.Text = Convert.ToString(AmountPaid);
        }
    }
}
